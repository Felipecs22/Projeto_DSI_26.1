...
function InfoRow({ label, value, onEdit, onDelete, styles, colors }: any) {
  return (
      ...
      <View style={styles.infoActions}>
        {onEdit ? (
          <TouchableOpacity style={styles.iconBtn} onPress={onEdit}>
            <Ionicons name="pencil" size={14} color={colors?.ACCENT ?? '#00d394'} />
          </TouchableOpacity>
        ) : null}
        {onDelete ? (
          <TouchableOpacity style={[styles.iconBtn, styles.iconBtnDanger]} onPress={onDelete}>
            <Ionicons name="trash-outline" size={14} color={colors?.DANGER ?? '#e53e3e'} />
          </TouchableOpacity>
        ) : null}
      </View>
    </View>
...
export default function ProfileScreen() {
  ...
  const handleDeleteReview = (reviewId: string, gameName: string) => {
    Alert.alert(
      'Excluir review',
      `Tem certeza que quer excluir sua review de "${gameName}"?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              showToast('Review excluída com sucesso!');
            } catch (error: any) {
              showToast(error?.message ?? 'Não foi possível excluir a review.', 'error');
            }
          },
        },
      ],
  ...
  const handleClearLibrary = async () => {
    ...
    try {
      ...
      await loadProfileData();
      showToast('Biblioteca limpa com sucesso!');
    } catch (error: any) {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <SectionCard title="Informações pessoais" icon={<Ionicons name="person-outline" size={16} color={colors.TEXT_MUTED} />} styles={styles}>
          <InfoRow
            ...
            colors={colors}
          />
          <InfoRow label="E-mail" value={email} styles={styles} colors={colors} />
          <InfoRow
            label="Usuário"
            value={username}
        ...
        <SectionCard title="Minhas reviews" icon={<Ionicons name="star-outline" size={16} color={colors.TEXT_MUTED} />} styles={styles}>
            ...
            userReviews.slice(0, 5).map((review) => (
              <View key={review.id} style={styles.reviewRow}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleDeleteReview(review.id, review.game)}
                >
          ...
          <Pressable style={styles.modalSheet} onPress={() => {}}>
            ...
            {avatarId ? (
              <TouchableOpacity style={styles.removeAvatarBtn} onPress={() => { saveAvatar(null); setAvatarPickerVisible(false); }}>
                <View style={styles.removeAvatarContent}>
                  <Ionicons name="trash-outline" size={16} color={colors.DANGER} />
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  removeAvatarContent: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  actionRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
