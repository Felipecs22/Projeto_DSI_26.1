...
export default function FriendsScreen() {
  ...
  const handleRemoveFriend = (item: FriendListItem) => {
    Alert.alert(
      'Remover amigo',
      `Tem certeza que quer remover ${item.user.displayName} da sua lista de amigos?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await friendService.removeFriend(user!.uid, item.relation.id);
              await loadFriendsData();
            } catch (error: any) {
            ...
            {friends.length === 0 ? (
              ...
              friends.map((item) => (
                <FriendItem
                  ...
                  actionLabel="Remover"
                  actionTone="danger"
                  onAction={() => handleRemoveFriend(item)}
